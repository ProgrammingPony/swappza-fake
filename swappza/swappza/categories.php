...
<html>
    <head>
        ...
        <script>
            ...
            var nonceToken = '<?php echo oab_create_nonce($mysqli, null, 'categories.php'); ?>';
            var FILE_NAME = 'categories.php';
        </script>
    </head>
    <body>        
        ...
        <div class="full-width-container centered-container body-background">            
            ...
            <!--Content goes here-->
            <?php 
            $categoryTree = oab_getSwappzaCategoryTree($mysqli);
            
            //PRINT HTML CODE WITH CATEGORY VALUES
            //Basic example:
            echo '<ul>';
            
            foreach ($categoryTree as $category) {
                echo "<li>categoryID={$category->getID()} categoryName={$category->getName()}</li>";
                echo '<li >';
                echo '<ul>';
                
                foreach ($category->getSubCategories() as $subcategory) {
                    echo "<li>SubCategoryID={$subcategory->getID()} SubCategoryName={$subcategory->getName()}</li>";
                }
